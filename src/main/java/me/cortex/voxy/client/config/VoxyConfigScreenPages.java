    ...
    public static OptionPage page() {
        ...
        groups.add(OptionGroup.createBuilder()
                .add(OptionImpl.createBuilder(boolean.class, storage)
                        ...
                        .setTooltip(Component.translatable("voxy.config.general.maxVramUsage.tooltip"))
                        .setControl(opt -> new SliderControl(opt, 512, 16384, 512, v -> Component.literal(v + " MB")))
                        .setBinding((s, v) -> s.maxVramUsageMB = v, s -> s.maxVramUsageMB)
                        .setFlags(OptionFlag.REQUIRES_RENDERER_RELOAD)
                        .build()
                ).add(OptionImpl.createBuilder(int.class, storage)
                        .setName(Component.translatable("voxy.config.general.targetFPS"))
                        .setTooltip(Component.translatable("voxy.config.general.targetFPS.tooltip"))
                        .setControl(opt -> new SliderControl(opt, 30, 240, 10, v -> Component.literal(Integer.toString(v))))
                        .setBinding((s, v) -> s.targetFPS = v, s -> s.targetFPS)
                        .build()
                ).add(OptionImpl.createBuilder(boolean.class, storage)
